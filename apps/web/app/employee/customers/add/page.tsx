...
export default function EmployeeAddCustomerPage() {
  ...
  useEffect(() => {
    fetch("/api/customers?region=all")
      .then(async (response) => {
        if (!response.ok) throw new Error("Failed to load existing customers.");
        const payload = (await response.json()) as {
          customers: Array<{ fullName?: string; boxNumber?: string; building?: string }>;
        };
  ...
  const existingCustomerNames = useMemo(
    () => Array.from(new Set(allCustomers.map((c) => String(c.fullName ?? "").trim()).filter(Boolean))),
    [allCustomers]
  );
    ...
    <AppShell title="Add Customer" subtitle="Create a new customer record" navItems={employeeNavItems}>
      ...
      <div className="card">
        ...
        <SimilarValueGuard
          label="customer"
          value={fullName}
          candidates={existingCustomerNames}
          onUseExisting={(existing) => setFullName(existing)}
        ...
        <div className="card-actions-right">
          <button
            type="button"
            onClick={onSubmit}
            disabled={isSubmitting || boxNameBlocking || buildingNameBlocking || nameBlocking}
            title={
              boxNameBlocking || buildingNameBlocking || nameBlocking
                ? "Confirm the highlighted name(s) above first"
                : undefined
            }
          >
