...
function LossPrintaniaReportContent() {
  ...
  const { monthKey, setMonthKey, region } = useReportScope({ searchParams });
  ...
  const storageEntryKey = monthKey;

  useEffect(() => {
    let cancelled = false;
    fetchAppCalculatedKwh(monthKey).then((value) => {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all(TREND_MONTHS.map(async (month) => [month, await fetchAppCalculatedKwh(month)] as const)).then(
      (entries) => {
        if (cancelled) return;
  ...
  return (
    <AppShell
      title="Actual vs Reported - Printania"
    ...
    >
      ...
      <div className="card">
        <ReportScopeFilters
          idPrefix="loss-printania"
          monthKey={monthKey}
          onMonthChange={setMonthKey}
          region={region}
          onRegionChange={() => undefined}
          allRegionsLabel="Printania"
        />
        <div style={{ marginTop: 8 }}>
          <ReportScopeLabel monthKey={monthKey} region="printania" />
        </div>
      </div>
      <div className="card">
